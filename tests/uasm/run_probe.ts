...
function prepareProbeDirs(): { inputDir: string; outputDir: string } {
...
function readProbeResults(outputDir: string, logFile: string): ProbeResults {
  ...
  try {
    return JSON.parse(rawJson) as ProbeResults;
  } catch (parseErr) {
    throw new Error(
      `Failed to parse ${resultsPath}: ${parseErr instanceof Error ? parseErr.message : parseErr}\n` +
        `First 200 chars: ${rawJson.slice(0, 200)}`,
    );
  }
}

function printProbeReport(results: ProbeResults): void {
  ...
  console.log(
    `udonAssembly field exists:    ${results.udonAssemblyFieldExists}`,
  );
  console.log(
