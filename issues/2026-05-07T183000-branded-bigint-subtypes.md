...
The `stub-bigint-migration` issue migrated bare `number` → `bigint` for integer-typed APIs. While this eliminates unnecessary Double→Int32 conversions at the EXTERN boundary, it loses compile-time type safety: callers cannot distinguish `UdonInt` from `UdonLong` at the TypeScript level.
